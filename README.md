...
1) Heurística #1 - Visibilidade do Status do Sistema
   - Implementada através das mensagens:
     [Passo 1 de 3] Seleção de Item
     [Passo 2 de 3] Quantidade
     [Passo 3 de 3] Confirmação do Pedido
   - O usuário sempre sabe em qual etapa está.

2) Heurística #3 - Controle e Liberdade do Usuário
   - O sistema permite digitar:
     "voltar"   -> retorna para a etapa anterior
     "cancelar" -> aborta o pedido a qualquer momento
   - O usuário pode corrigir erros sem reiniciar o programa.

3) Heurística #9 - Ajuda no Reconhecimento e Tratamento de Erros
   - Quando o usuário digita um código inválido, o sistema mostra
     uma mensagem clara:
     "Código X não encontrado. Nossos códigos vão de 1 a 10."
   - Também há validação para entrada não numérica e quantidade inválida.
